 ...
 * 线下优惠券领取请求控制器。
 * @author 胡睿。
 * CreateTime:2015/05/27 18:39:36.
 */
class CouponRequestAction extends LoginRequestAction {
	/**
	 * 用户提交领取优惠券。
	 */
	public function getCouponConfirm() {
		// 常规全局变量参数
		$customer_id = $_SESSION ['currentcustomer'] ['customer_id']; 			// 提交领取优惠券的顾客
		$openid = $_SESSION ['currentwechater'] [$this->eid] ['openid']; 		// 获取当前微信用户的openid
		
		// step1、对于优惠券本身是否存在的校验
		// 1)优惠券coupon_id字段不为空
		$coupon_id = I ( 'coupon_id', '' ); // 需要领取的优惠券id
		if (empty ( $coupon_id )) {
