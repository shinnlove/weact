...
define('lib/global', ['module/netType', 'lib/jquery.min'], function(require, exports, module) {
    ...
    jQuery.extend({
        DIC: {
            ...
            /**
             * 设置cookie函数。
             * js:setTime(n)，设置一个距离1970年1月1日n秒的日期；DateObject.getTime()，返回距 1970年1月1日之间的毫秒数。
             * @param String cookieName cookie的键值
             * @param String cookieValue cookie键值对应的值
             * @param int seconds cookie过期时间
             * @param String path cookie保存路径
             * @param String domain cookie允许跨域读取的域名
             * @param String secure cookie的安全时间
             */
            setcookie: function (cookieName, cookieValue, seconds, path, domain, secure) {
                var expires = new Date();
                expires.setTime(expires.getTime() + seconds); // getTime()方法可返回距 1970年1月1日之间的毫秒数；再加上要保存的秒数，就是未来过期的时刻；cookie的expires再toGMTString()即可
            ...
            /**
             * 读取cookie函数。
             * @param String name 要读取的cookie的名称
             */
            ...
            getQuery: function(key) {
                ...
                if (search.indexOf('?') != -1) {
                    ...
                    return jQuery.DIC.parseStr(str, key); // 进行解析
                }
            },
            reload: function(href, timeout) {
                ...
                setTimeout(function() {
                    if (re.test(href)) {
                        ...
                        window.location.href = window.DOMAIN.substr(0, window.DOMAIN.length - 1) + href;			//注意这里，域名要正确
                    } else {
             ...
             * @param maxlen 最大允许显示长度，这是英文字符长度
            ...
            mb_cutstr: function(str, maxlen, dot) {
                ...
                for(var i = 0; i < str.length; i++) {
                    len += str.charCodeAt(i) < 0 || str.charCodeAt(i) > 255 ? (jQuery.DIC.charset.toLowerCase() === 'utf-8' ? 3 : 2) : 1; // 英文西欧字符长度1；utf-8中文长度3；gbk和gb2312中文长度2。
                    if(len > maxlen) {
             ...
             * @param maxlen 最大可以输入的字符数
             */
            strLenCalc: function(obj, showId, maxlen) {
                var v = obj.value, maxlen = !maxlen ? 200 : maxlen, curlen = maxlen, len = jQuery.DIC.strlen(v); // 默认可以输入200字符
            ...
            _dialog: function(opts) {
                ...
                switch (opts.icon) {
                    case 'success':
                        iconClass = 'icon_success'; // 成功css图标，如：jQuery.DIC.dialog({content:result.message, icon:'success', autoClose:true});
                        break;
                    case 'none':
                        iconClass = ''; 			// 不要图标
                        break;
                    case 'error':
                    	iconClass = ''; 			// 错误css图标，如：jQuery.DIC.dialog({content:result.errmsg, icon:'error', autoClose:true});
                    	break;
                    default:
                ...
                if (isHtml) {
                    ...
                    if (!opts.title && !btnOk && !btnCancel) {
                    	// 无标题、无确认或取消，简单的弱提示，如：jQuery.DIC.dialog({content:msg, autoClose:true});
                        dialogHtmlArr.push('<div style="position:fixed;z-index:' + dialogZIndex +';" id="' + dialogId + '"><div class="tips br5">'); // 追加提示层
                        if (dId == 'loading') {
                            // 如果是loading等待，如：jQuery.DIC.dialog({id:'loading'});
    ...
    //                }
            ...
            showLoading: function(display, waiting, autoClose) {
                ...
                if (display == 'block') {
                    //jQuery.DIC.dialog({id:'loading', content:waiting, noMask:true, autoClose:autoClose});	//不遮罩显示loading
                    jQuery.DIC.dialog({id:'loading', content:waiting, isMask:false, autoClose:autoClose});	//不遮罩显示loading
                } else {
                    jQuery.DIC.dialog({id:'loading'});														// 关闭loading对话框
                }
            },
            /**
             * ajax请求数据函数。
            ...
            ajax: function(url, data, opts) {
                ...
                var ajaxOpts = {
                    ...
                    success: function(result, jqStatus, curXHR) {
                        ...
                        jQuery.DIC.showLoading('none'); // 关闭对话框
                        ...
                        if (result.errCode == 0) {
                        	// 是否要提示成功
                            if (!opts.noMsg) {
                                jQuery.DIC.dialog({content:result.message, icon:'success', autoClose:true}); // 提示成功
                            }
                            // 是否需要跳转
            ...
            ajaxForm: function(formId, opt, isSubmit) {
                var opt = opt || {};
                var loadingTimer = '';
                var url = opt.url || jQuery('#' + formId).prop('action'); 		// 抓取表单要提交的action地址
                // 处理表单提交url
                url = url.indexOf('?') === -1 ? url + '?' : url + '&';
                url = url.replace(/\&resType\=[^\&]+/g, '') + 'resType=json';
                url = url.replace(/\&isAjax\=1/g, '') + '&isAjax=1';
            ...
            //增加点击阴影，如果不指定样式，默认使用commBg，一块半透明方块黑色
            touchStateNow: function(obj, style) {
                style = style || 'commBg';
                obj.addClass(style);
                setTimeout(function() {
                    obj.removeClass(style);			//0.1秒撤销阴影，看起来像点了一样
                }, 100);
            },
