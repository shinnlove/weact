...
(function($) {
	...
	/**
	 * 扩展树表格级联勾选方法：（已经经过我修改正确 2014/10/21 01:28:25）
	...
	$.extend($.fn.treegrid.methods,{
		...
		cascadeCheck : function(target,param){
			...
			var status = false;							//用来标记当前节点的选中状态，true:勾选，false:未勾选
			
			var children = null;
			var parent = null;
			
			var level = $(target).treegrid('getLevel', param.id);
			if(level > 1){
				parent = $(target).treegrid('getParent', param.id);					//孩子节点，尝试获取它的父节点对象
			}else{
				children = $(target).treegrid('getChildren', param.id);				//父节点，尝试获取它的孩子节点
			}
			
			/* var children = $(target).treegrid('getChildren', param.id);				//尝试获取它的孩子节点
			var parent = $(target).treegrid('getParent', param.id);					//尝试获取它的父节点对象 */
			var selectNodes = $(target).treegrid('getSelections');					//获取当前全部被选中项
			
			var siblings = 0;							//初始化兄弟结点数目（只有是当前所选结点为孩子节点的时候才有兄弟）
			
			var parentmustchecked = false;				//如果是孩子节点，父节点是否被选中标记
			
			//注意：level > 1代表所选是孩子节点
			if(level > 1){
				siblings = $(target).treegrid('getChildren', parent[idField]);		//获取其兄弟节点
				var ischecked = false;					//初始化比对标记
				var totalcheck = 0;						//孩子节点被选中的个数
				for(var j=0; j<siblings.length; j++){
					//将当前所有兄弟节点一一取出，与所选中的行进行循环比对
					ischecked = false;					//默认当前要比对的结点没有被选中
					for(var k=0; k<selectNodes.length; k++){
						//如果兄弟被选中
						if(siblings[j][idField] == selectNodes[k][idField]){
							ischecked = true;
							totalcheck += 1;			//总的孩子节点被选中个数+1
							break;
						}
					}
				}
				if(totalcheck < siblings.length){
					//总孩子结点被选中的个数小于兄弟结点个数，并未完全选择
					//alert('还有孩子节点没被选中');
					parentmustchecked = false;
				}else{
					//alert('所有孩子节点都被选中');
					parentmustchecked = true;
				}
			}else{
				//所选结点是父亲节点，则进行展开结点
				var current_leaves = $(target).treegrid('getChildren', param.id);	//尝试获取其孩子节点数组
				
				if(current_leaves && current_leaves.length > 0){
					//已经展开过，展开并选择
					$(target).treegrid('expand', param.id);
				}else{
					$(target).treegrid('expand', param.id);
					$(target).treegrid('unselect', param.id);		//第一次展开，展开后取消选择
				}
			}
			
			for(var i=0;i<selectNodes.length;i++){
				if(selectNodes[i][idField] == param.id) {
					status = true;						//当前结点是否被选中
				}
			}
			
			//级联选择父节点
			//selectParent(target[0],param.id,idField,status);						//原来的代码
			selectParent(target[0],param.id,idField,parentmustchecked);				//现在的代码（已经成功），选父亲的时候status状态是parentmustchecked
			selectChildren(target[0],param.id,idField,param.deepCascade,status);	//原来的代码
			/**
			 * 级联选择父节点
			 * @param {Object} target
			...
			function selectParent(target,id,idField,status){
				...
				if(parent){
					var parentId = parent[idField];							//获取当前parent的id
					if(status){
