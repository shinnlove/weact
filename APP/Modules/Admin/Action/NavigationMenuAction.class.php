 ...
 *
 */
class NavigationMenuAction extends PCViewLoginAction {
	...
	/**
	 * （新改过了）编辑导航视图页面。
	 * 导航挂接规则比喻：新·空当接龙。！！！真正理解菜单挂接规则，会体会到蜘蛛纸牌这个比喻是如此的形象！
	 * 页面按照该规则初始化后，给导航编辑的提交提供了大量的便捷与简化。
	 * Author：赵臣升。
	 * CreateTime：2014/09/12 20:03:25.
	 * 编辑导航的页面，接收参数后初始化一些值。
	 * 包括：
	 * 1、导航性质；2、导航类别；3、导航。
	 *
	 * 编辑页面  上级导航菜单  下拉框值  的设计思路：（需要一个nav_id和nav_name）
	 * 编辑页面打开的时候，首先读取当前导航的类型，将所有同类的一级导航查询出来（二级导航不必要，因为不能在二级导航下再设置导航）。
	 * if(当前编辑导航是顶级导航){
	 *     if(当前导航下有子级导航){
	 *         //只显示一个无上级导航（因为他不能挂到任何一个顶级导航下，否则会成为三级导航，最大支持二级）
	 *     }else{
	 *         //当前导航下没有子级导航（说明它是一个空顶级导航、或是直接关联详情页面||商品陈列||超链接导航）
	 *         //则他可以挂到任何一个（下面所述条件的）顶级导航下，成为二级导航，所属的详情||商品||超链接依然存在
	 *         //下面所述条件：下拉框中不出现他本身、且满足没有详情或商品或超链接的顶级导航（该顶级导航可以是空，也可以是只有二级导航）
	 *     }
	 *     //这种情况它自己的值不用出现在下拉框中
	 * }
	 * else{
	 *     //当前导航不是顶级导航，更改父级导航，必须调整到没有详情或商品且不是超链接的父级导航下（该父级导航可以是空，也可以是只有二级导航）
	 *     //下拉菜单初始化值是  当前父级导航的名字，并且出现所有满足没有详情或商品或超链接的顶级导航（该顶级导航可以是空，也可以是只有二级导航）
	 * }
	 */
	public function editNavigation(){
		//Step1：查询当前导航的信息
		$edit_nav = array (
				$this->cc_nav_id => I ( 'nav_id', '-1' ), // 接收要编辑的导航
				$this->cc_e_id => $_SESSION ['curEnterprise'] ['e_id'], // 当前商家
				$this->cc_is_del => 0 // 没有被删除的
		);
		$navtable = M ( $this->table_name );
		$navinfo = $navtable->where ( $edit_nav )->find ();
		if (! $navinfo) $this->error ( "不能编辑不存在的导航！" ); // 容错处理
	
		// 对导航信息进行处理：
		if ($navinfo [$this->cc_fathernav_id] == '-1') {
			$navinfo ['level_name'] = '顶级导航';
		} else {
			$navinfo ['level_name'] = '二级导航';;
		}
		if ($navinfo [$this->cc_nav_type] == 1) {
			$navinfo ['attribute'] = '信息类'; // 信息类导航
		} else if ($navinfo [$this->cc_nav_type] == 2) {
			$navinfo ['attribute'] = '服装类'; // 服装类导航
		} else if ($navinfo [$this->cc_nav_type] == 3) {
			$navinfo ['attribute'] = '超链接'; // 超链接导航
		} else if ($navinfo [$this->cc_nav_type] == 4) {
			$navinfo ['attribute'] = '餐饮类'; // 餐饮类导航
		} else if ($navinfo [$this->cc_nav_type] == 5) {
			$navinfo ['attribute'] = '商品类'; // 商品类导航
		}
		$navinfo [$this->cc_nav_imagepath] = assemblepath ( $navinfo [$this->cc_nav_imagepath] ); // 导航图片路径的处理
		$this->navinfo = $navinfo; // 推送要编辑的导航信息
	
		// Step2：初始化上级下拉菜单值的数组  $father_nav
		$father_nav = array (
				0 => array (
						$this->cc_nav_id => '-1',
						$this->cc_nav_name => '无上级导航'
				), // 初始化原始下拉框的值
		);
		if (! $this->navExistChild ( $navinfo )){
			// 如果当前导航下边没有子级导航（包括无子级导航的顶级导航和二级导航），最多支持两级导航挂接，并且不能带孩子或内容
			$cadidatemap = array (
					$this->cc_e_id => $_SESSION ['curEnterprise'] ['e_id'],	// 当前商家下的
					$this->cc_fathernav_id => '-1',							// 要找可挂接的顶级导航
					$this->cc_is_del => 0									// 没有被删除的
			);
			if ($navinfo [$this->cc_nav_type] == 3){
				$cadidatemap [$this->cc_nav_type] = array ( 'neq', 3 );		// 如果当前导航是超链接导航，可挂接到满足条件的顶级商品、信息导航下
			} else {
				$cadidatemap [$this->cc_nav_type] = $navinfo [$this->cc_nav_type];	// 如果是信息或者商品类的导航，则要跟当前导航同类的导航（很重要！非同类导航不能挂接）
			}
			if ($navinfo [$this->cc_fathernav_id] == '-1') {
				$cadidatemap [$this->cc_nav_id] = array ( 'neq', $navinfo [$this->cc_nav_id] ); // 如果编辑的是顶级导航，下拉菜单中排除它自己
			}
			$nav_cadidate = M ( $this->table_name )->where ( $cadidatemap )->select ();	//尝试找出可能满足条件的顶级菜单
			if ($nav_cadidate) {
				for($i = 0; $i < count ( $nav_cadidate ); $i ++) {
					if ($nav_cadidate [$i] [$this->cc_nav_type] != 3 && $this->topNavEmpty ( $nav_cadidate [$i] ) ) {
						array_push ( $father_nav, $nav_cadidate [$i] );		//对没有信息详情或商品详情的顶级导航（除超链接导航外）菜单再进行一次过滤
					}
				}
			}
		}
		$this->father_nav = $father_nav;									//推送满足条件的顶级菜单
	 ...
	 * （暂无试用，导航已关闭）
	 */
	public function suspenNavigation(){
		$this->display();
	}
	
	/**
	 * 后台二级导航模板页面。
	 * Function：直接传入当前路径和商家编号进行模板查询，然后展示页面。
	 */
	public function secondNavigation() {
		$tplinfo = array (
