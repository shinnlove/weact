...
//        }
	...
	/**
    * 关闭子帐号
    * @param SubAccountID 子帐号id
    */
	  function closeSubAccount($SubAccountID)
	  {
        //主帐号鉴权信息验证，对必选参数进行判空。
        $auth=$this->accAuth();
        if($auth!=""){
        ...
        // 拼接请求包体
        if($this->BodyType=="json"){
           $body= "{'subAccountSid':'$SubAccountID'}";
        }else{
           $body="<SubAccount>
                    <subAccountSid>$SubAccountID</subAccountSid>
                  </SubAccount>";
        }
        $this->showlog("request body = ".$body);
        // 大写的sig参数  
        $sig =  strtoupper(md5($this->AccountSid . $this->AccountToken . $this->Batch));
        // 生成请求URL
        $url="https://$this->ServerIP:$this->ServerPort/$this->SoftVersion/Accounts/$this->AccountSid/CloseSubAccount?sig=$sig";
        $this->showlog("request url = ".$url);
        // 生成授权：主帐号Id + 英文冒号 + 时间戳
        $authen = base64_encode($this->AccountSid . ":" . $this->Batch);
        // 生成包头 
